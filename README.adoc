...
* PC Port save files (*.sav) for {uri-gh}/blawar/ooot[Open Ocarina^] (oot.sav) and {uri-gh}/HarbourMasters/Shipwright[Ship of Harkinian^] (only Deckard Alfa 1.0.0 and Roy Alfa 2.0.0) (oot_save.sav).
* Dolphin save files (*.gci).
* RetroArch and other Libretro SaveRAM files (*.srm, *.SaveRAM).
* Mupen64Plus and Mupen64Plus FZ saves and states (*.v2.sav, *.st*, *.m64p).
...
* *Regions*: Make sure to select the correct region for the ROM used, otherwise Japanese characters or squares may appear in the player names.
